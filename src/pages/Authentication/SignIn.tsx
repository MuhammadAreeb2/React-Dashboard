  ...
  const SignInGoogle = async () => {
    try {
      const authResponse = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: "http://localhost:5173/profile", // Change to your server endpoint
        }
      });

      if (authResponse.error) {
        console.error('Error during Google sign-in:', authResponse.error.message);
      } else if (authResponse.data && authResponse.data.url) {
        await console.log(authResponse.data)
        const googleToken = authResponse.data.url;

        // Set the token in local storage
        // localStorage.setItem('token', googleToken);

        // Set the token in the state if needed
        // setToken(googleToken);

        console.log("Login successful");
      } else {
        console.error('No token found in the authentication response.');
      }
    } catch (error) {
            ...
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              ...
              <form onSubmit={handleSignIn}>
                ...
                <div className='text-right py-5'>
                  {/* <button className='text-center' onClick={resetPassword}> forget passsowrd</button> */}
                  <Link
                    to="/forgotpassword"
                    className="text-sm text-primary hover:underline focus:outline-none focus:text-primary dark:text-primary-dark"
                  >
                    Forgot password?
                  </Link>
                </div>
